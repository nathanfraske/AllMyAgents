...
const MANAGER_TASK_ACCOUNTABILITY_RULES = [
  ...
  'For remote builds, pass required_tools to start_run. A missing tool is a provisioning action, not a blocker to merely report: find this project\'s reviewed setup recipe (or reviewed bootstrap recipe) and pass its exact command as setup_command. The hub records it as a separate durable prerequisite, queues the requested run behind success, and verifies the tools again. Do not infer packages, run an unreviewed installer, or invent another dependency manifest.',
]
...
export class SessionManager {
  ...
  setDurableRunController(controller: DurableRunController): void {
    ...
    controller.setRemoteExecutor(async (run, target) => {
      ...
      if (target.requiredTools?.length) {
        const inspected = await this.remoteDeviceExecute(run.targetSessionId, target.siteId, {
          op: 'inspect',
          rootId: target.rootId,
        }, { durableRunId: run.id })
        if (!inspected.ok || !inspected.environment) return inspected
        const missing = target.requiredTools.filter((tool) => inspected.environment?.tools[tool] !== true)
        if (missing.length) {
          return {
            ok: false,
            error: `Remote run dependencies are still unavailable after preflight: ${missing.join(', ')}. The command was not started.`,
            failure: { stage: 'target', code: 'REMOTE_DEPENDENCIES_MISSING' },
            telemetry: inspected.telemetry,
          }
        }
      }
  ...
  private materializeSessionInstructions(
    ...
    const overseerText = record.isOverseer
      ? [
          ...
          'For remote work, inspect the environment with the intended required_tools, then pass those requirements to start_run. If tools are missing, do not merely report the inventory as a blocker: find the project\'s reviewed setup recipe (or reviewed bootstrap recipe) and pass its exact command as setup_command. The hub records provisioning as a separate durable prerequisite, queues the requested build/test behind its success, and verifies the tools again before launch. An operator-authorized whole testbed grant is complete standing authority for all roots/capabilities advertised by that node; do not ask for another directory, checkbox, approval, or project attachment. For a project, the hub automatically creates or reconciles an app-owned checkout beneath the selected machine root and verifies published-source parity before each durable run. Explicit remote grants permit normal target concurrency. Do not infer packages, run an unreviewed installer, or invent another dependency manifest.',
  ...
  private async sendAdmitted(
    ...
    if (record.status === 'active' || record.status === 'starting' || this.executor.isBusy(sessionId)) {
      ...
      if (!carriesOperatorAuthority) {
        ...
        // Establish the authority transition at the provider's steer boundary, never before it. Until
        // steer() accepts this exact input, the current teammate-origin turn stays clamped and the durable
        // queue is its lossless fallback. Once accepted, provider semantics place the operator message at
        // the current/next tool boundary. Work admitted after that linearization point is therefore driven
        // by authenticated operator input; work already issued or awaiting approval is not re-evaluated.
        // Removing the fallback at the same hub boundary also keeps the message from resurfacing as a
        // second, apparently-old prompt after the turn completes.
        try {
          admission.markDispatched()
          ...
          this.persist(record)
          this.busTurnSessions.delete(sessionId)
          this.operatorTurnSessions.add(sessionId)
          this.journal.append(sessionId, 'session/steered', {
            text,
            attachments,
            source: 'operator',
            authority: 'operator-from-provider-boundary',
            operatorInterventionId: queued.id,
          })
          ...
          // The provider may have ended the turn between the busy check and steer admission. Keep the
          // already-persisted operator input and start it exactly once as a fresh operator turn when the
          // lifecycle becomes idle; the operator never has to notice the race or retype the message.
          this.journal.append(sessionId, 'session/operator-turn-deferred', {
            deferredOperatorTurnId: queued.id,
            reason: 'the provider did not accept an immediate operator steer',
            queuedAt: queued.queuedAt,
            delivery: 'fresh-turn-fallback',
            message: error instanceof Error ? error.message : String(error),
          })
          this.journal.append(sessionId, 'session/operator-authority-not-conferred', {
            deferredOperatorTurnId: queued.id,
            message:
              'The provider did not accept the operator steer boundary. The running turn kept its original authority and the exact input remains queued as a fresh operator-origin turn.',
          })
  ...
  async managerStartRun(
    ...
    try {
      ...
      const run = await this.durableRuns.start({
        ...
        ...(input.dependsOnRunId ? { dependsOnRunId: input.dependsOnRunId } : {}),
        kind: input.kind,
        executable,
        args,
        cwd: hostCwd,
