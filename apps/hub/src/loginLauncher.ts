...
// Opens a NEW VISIBLE terminal window that runs the vendor login with the right config-dir
// env var pointed at profileDir, so the browser OAuth flow can open for the user. The hub
// runs headless, so we shell out to cmd's `start` builtin (CREATE_NEW_CONSOLE) — the only
// reliable way to guarantee a visible console from a windowless daemon. Fire-and-forget:
// awaitLogin() observes the result by polling for the credentials file.
export function startLogin(provider: Provider, profileDir: string): void {
  fs.mkdirSync(profileDir, { recursive: true })

  if (process.platform !== 'win32') {
    // Non-Windows: a headless daemon cannot reliably pop a visible terminal. The server
    // layer returns the manual `pnpm login:` instruction instead of calling this path.
    return
  }
