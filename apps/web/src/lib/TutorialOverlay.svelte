<script lang="ts">
  ...
  const projectSteps: StepCopy[] = [
    ...
    },
    {
      anchor: TUTORIAL_ANCHORS.newProjectFlow,
      eyebrow: 'ISOLATION',
      title: 'Project folder or worktree',
      body: 'An agent can work directly in the project folder, or use a worktree—an isolated branch and folder that keeps simultaneous changes apart. Worktrees are safer for parallel work; direct work is simpler for one agent.',
    },
    {
      anchor: TUTORIAL_ANCHORS.newProjectFlow,
      eyebrow: 'THE TEAM',
      title: 'Choose how agents work together',
      body: 'A manager-led team coordinates delegated agents. Independent agents run their own starting tasks without answering to a manager. Either choice is optional, and you can use both or neither.',
    },
    {
      anchor: TUTORIAL_ANCHORS.newProjectFlow,
      eyebrow: 'SAFE TO EXPLORE',
      title: 'Nothing is created until Finalize',
      body: 'You can move through the choices and go back without creating a project, cloning a repository, or starting an agent. The app commits the setup only when you finalize the launch.',
    },
  ]
