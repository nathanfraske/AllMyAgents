...
**D13.1 — Mesh remote access via AllMyStuff "sites" (coexistence spec, 2026-07-23).** Confirmed on the dev machine: the full stack runs (allmystuff-serve + myownmesh + GUI), control sockets up (`\\.\pipe\allmystuff-node`, `\\.\pipe\myownmesh.sock`), fleet network "Nathan's Fleet" (polar-heisenberg) active with peers. MyOwnMesh is P2P WebRTC data channels — NO routable IP, NO CLI port-forward — so the hub's web UI rides the mesh as an AllMyStuff **site** (AllMyStuff's built-in web-service-over-mesh; "we do not need Tailscale"). Integration, playing nice by construction with a running stack:
- **Never fork, always attach.** The hub NEVER spawns its own myownmesh/allmystuff-serve, never sets MYOWNMESH_HOME, never touches mesh identity. The control-socket bind is the singleton lock (a 2nd node refuses: "another allmystuff node already owns this machine's control socket") → zero contention by construction.
- **Probe → reuse → (absent only) spawn.** On startup probe the allmystuff-node control socket; if it answers, REUSE it and own nothing; else ensure a node via the bundled+pinned sidecar (D13), then register. Additive either way.
- **Additive, self-cleaning site.** Register the hub as a site = one control-socket command adding an endpoint beside the node's screen/terminal/files caps; deregister on shutdown; a reused node keeps running untouched, only a self-spawned (headless) node is torn down (CECSupport lifetime rules).
- **Hub stays pure loopback.** Keeps binding 127.0.0.1:7777, never a network interface; the site proxies to loopback, gated by mesh device-auth + our own device token → nothing exposed except through the authenticated site.
- **Delegated updates** (never touch a node we don't own). 
- **Sites-API research — RESOLVED 2026-07-23** (verbatim source read across mrjeeves/AllMyStuff). A *site* is a **transparent layer-4 (raw TCP) tunnel** over the mesh — `node/src/sites.rs`: "no HTTP parsing, no idle timeout, full-duplex", and the module doc explicitly names **WebSocket / SSE / keep-alive / gRPC / DB wire protocols** as carried unchanged. So the hub's WS stream on loopback tunnels end-to-end — the earlier HTTP-only worry is moot. **Registration = one control-socket command**, not a CLI (there is no `site` subcommand) and not a static file for a live node (`allmystuff-sites.json` is read once at startup): connect the app control socket (`\\.\pipe\allmystuff-node`; Unix `${MYOWNMESH_HOME||~}/.myownmesh/allmystuff-node.sock`), send framed JSON `{cmd,args}` where the frame is `[u32 BE len][tag=0 JSON][payload]` and **len includes the tag byte**, reply is `{ok,result,error}`. Call `site_set_exposed {exposed:{"tcp:7777":"<label>"}}` — but it **REPLACES the whole map**, so read `site_exposed` first and merge (never clobber the node's screen/terminal/files caps). Auth: **same-owner fleet devices need no grant** (`allmystuff-graph/src/authz.rs`: "your own devices need no grant to talk to each other"); the port allow-list *is* the exposed set (host refuses any unadvertised port). Peers pull — `site_map {node,port}` → `{localPort}`, direct same-number when the port is ≥1024 and free, else remapped from 47000 — then open `http://localhost:7777`.
- **BUILT (hub side) 2026-07-23**: `apps/hub/src/meshSite.ts` — a `net`-based control-socket client implementing the exact framing, `register()` (read-merge-write, never clobbers), `deregister()` (re-reads, best-effort, on clean shutdown), `setEnabled()` runtime toggle, and `status()`. Wired in `index.ts` (opt-in via `config.mesh.enable` or `MESH_EXPOSE=1`; SIGINT/SIGTERM deregister with a 2.5s guard) and `server.ts` (`GET/POST /api/mesh`). Graceful by construction: absent node → `nodePresent:false`, hub runs local-only, never spawns a node.
- **STILL PENDING before this is safe to leave ON by default**: (1) **device token** — the hub grants full control and today has NO auth, so a mesh-exposed hub trusts every fleet device; add a bearer/device token required on all `/api` + `/ws` when exposure is on, and wire the web UI + desktop app to present it. Until then exposure defaults OFF and is a deliberate toggle. (2) UI **"mesh" panel** showing the site address (`peerUrl`) + the enable toggle (queued behind the in-flight UI work that owns `apps/web`). (3) The **CECSupport sidecar bring-up (D13)** for the *absent-node* branch — right now absent node = local-only; ensuring a node when the user wants one is the D13 fetch/probe/spawn path. "Start a project on any PC" = the WSL/remote node runner (P3) reachable over the same mesh, or `amst`-launched.
