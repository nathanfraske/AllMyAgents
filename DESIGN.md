...
- **Server-side identity influence on "clean" profiles** — the `C:\Users\natha` incident: a fresh profile with zero identity in visible context emitted a path matching the user's *other* PC's home dir. Every local channel was clean (no email in context, empty memory paths, no MCP); explanation is either convergent name-truncation coincidence or account-side signals at the vendor (the request runs under the user's OAuth identity regardless of local isolation). Testable: N repeated sessions on a fresh profile — recurrence of the same name implies server-side influence. Implication either way: config-dir isolation isolates credentials and local state, NOT whatever the vendor associates with the account.
