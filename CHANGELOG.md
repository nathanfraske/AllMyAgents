...
- Successful, failed, cancelled, and outcome-unknown durable runs now enqueue one action-required result for the
  session that started them, including the exact run id and bounded terminal evidence.
- An idle manager wakes immediately. A result arriving during an active turn stays queued until idle and then
  starts one fresh continuation instead of racing a late steer into a turn that is already finishing.
- The continuation acknowledgement is durable and idempotent across hub ownership changes. A crash after saving
